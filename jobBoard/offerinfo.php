...
<main>
	...
	if (isset($_GET["offerid"])) {
		...
		//Gets the offer by id from the database
		$query = ("SELECT * FROM approved_offers WHERE job_id='$job_id'");
		$result = mysqli_query($conn, $query);

		$offer = mysqli_fetch_assoc($result);

		//Creates a job-card for the clicked offer
	?>
		...
		<ul class="jobs-listing">
			<?php
			//Gets all offers except the displayed one
			$query = "SELECT * FROM approved_offers WHERE job_id!=$job_id ORDER BY job_id DESC";
			...
			//Displays two offers as suggestions
